...
const ConfirmSchedulingScreen = ({ navigation, route }) => {
  const dispatch = useDispatch();
  const {params:{dateTime,seats}} = route;
  ...
  function backToHome() {
    return (
      <Text
        onPress={() => {
          dispatch(setScreen({screen:"Rides"}));
          navigation.push("BottomTabBar");
        }}
        style={{
      ...
      <View style={{ alignItems: "center", margin: Sizes.fixPadding * 2.0, flex:1 }}>
        ...
        <View style={{flex:1}}>
          ...
          <Text style={{ ...Fonts.blackColor14Medium, textAlign: "center" }}>
            {t('you have scheduled a ride')} {t('on')}{`\n`} {dateTime} {t('with')} {i18n.language !== 'kiny' ? seats+' '+t(seats > 1 ? 'seats' : 'seat') : t(seats > 1 ? 'seats' : 'seat')+' '+seats }
          </Text>
        </View>
...
export default ConfirmSchedulingScreen;
